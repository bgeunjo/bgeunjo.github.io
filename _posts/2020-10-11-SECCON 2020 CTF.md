...
This release note said that `TypeScript 3.8 ~` supports **ECMAScript Private Fields** which starts with `#` notation. I tried to refer `#flag` from the outside of containing class, but like note said, I can’t be accessed or even detected outside of the containing class . So, I came up with two ideas.
...
By the javascript code, `__classPrivateFieldSet` sets value of private fields to argument.
