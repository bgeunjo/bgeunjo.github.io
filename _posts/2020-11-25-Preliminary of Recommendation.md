...
여기서 유사도는 두 사용자가 얼마나 비슷한 아이템을 선호했는지를 기준으로 합니다. 사용자 기반에서는 한 사용자가 평가한 영화들의 평점들을 가지고 벡터로 표현합니다. 위의 표를 보면 준용의 평가 벡터는 `(5,4,-,-)`이고 승윤의 평가 벡터는 `(-,4,1,5)`입니다. 이 때 두 사용자간 유사도는 두 벡터 간 유사도로 정의합니다. 식으로 나타내면 다음과 같습니다 :
$$
hat()
$$
